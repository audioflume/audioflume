...
def detect_edit_points(audio_path: str):
    ...
    # First hit: first strong onset after the very beginning.
    ...
    positive_threshold = np.percentile(energy_delta, 95)
    negative_threshold = np.percentile(energy_delta, 5)

    # Intro end: first sustained energy lift in the first half.
    for index, delta in enumerate(energy_delta):
        time = rms_times[index]

        if 5 < time < duration * 0.5 and delta >= positive_threshold:
            add_point(points, "intro_end", time, delta / max_positive_delta)
            break

    # Drop: strongest energy jump in the middle of the track.
