...
export default function MusicPlayer() {
  ...
  const jumpToCuePoint = (marker: CuePointMarker | null) => {
    ...
    const nextProgress = Math.max(
      0,
      Math.min(1, marker.time / currentSong.duration),
    );
    waveformProgressRef.current = nextProgress;
    seekTo(currentSong, nextProgress, isPlaying);
    schedulePlayerCanvasDraw();
        ...
        <div className="filmwave-player-song">
          ...
          <div className="filmwave-player-song-copy">
            ...
            <div
              ...
              className="flex min-w-0 items-center gap-1.5 truncate text-xs text-[var(--text-subtle)] font-[320]"
            >
              {remotePlayingInAnotherTab ? (
                <>
          ...
          >
            {showWaveform ? (
              <div className="flex w-full min-w-0 items-center gap-4 overflow-visible">
                <span className="w-10 flex-shrink-0 text-right text-xs text-[var(--icon-color)] font-[320]">
                  ...
                  {visibleCuePoints.map((marker) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        />
                        <span className="pointer-events-none absolute bottom-[calc(100%+8px)] left-1/2 z-[80] flex -translate-x-1/2 translate-y-1 items-center whitespace-nowrap rounded-full border border-[var(--border)] bg-[var(--bg-primary)] px-2.5 py-1 text-[11px] text-[var(--text-primary)] opacity-0 shadow-[0_10px_30px_rgba(0,0,0,0.18)] transition duration-150 group-hover/player-cue-point:translate-y-0 group-hover/player-cue-point:opacity-100 font-[320]">
                ...
                <span className="w-10 flex-shrink-0 text-xs text-[var(--icon-color)] font-[320]">
                  {formatTime(duration)}
                </span>
              ...
              <div className="whitespace-nowrap text-xs text-[var(--icon-color)] font-[320]">
                {showFullCompactTime
        ...
        >
          ...
          {currentSong.audioUrl && (
            <a
              ...
              className="ml-2 inline-flex h-7 cursor-pointer items-center justify-center gap-2 rounded-none bg-[var(--text-primary)] px-3 text-[11px] leading-none text-[var(--bg-primary)] transition hover:opacity-80 font-[320]"
            >
              <span>License</span>
