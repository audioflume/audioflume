...
export default function CreatePlaylistModal({
  ...
  const busy = isCreating || isUploadingCover;
  ...
  async function handleCoverChange(file: File) {
    ...
    try {
      ...
      formData.append("name", name.trim() || "playlist");
      ...
      footer={
        <div className="flex w-full items-center justify-between gap-3">
          <button
            type="button"
            disabled={busy}
            className="flex h-8 cursor-pointer items-center gap-1.5 rounded-none px-2.5 text-xs font-[320] text-[var(--text-secondary)] transition hover:bg-[var(--bg-hover)] hover:text-[var(--text-primary)] disabled:cursor-default disabled:opacity-70"
          >
            Unsplash
            <SearchIconSmall />
          </button>
