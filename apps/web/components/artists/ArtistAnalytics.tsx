  ...
  return (
    <div className="filmwave-backend-section flex min-h-[104px] flex-col justify-between p-4">
      ...
      <div>
        ...
        <div className="mt-2 text-[10px] text-[var(--text-muted)] font-[320]">{note}</div>
      </div>
    </div>
  );
}
...
function ActivityChart({ timeline }: { timeline: ArtistAnalyticsTimelinePoint[] }) {
  ...
  if (!hasActivity) {
    return (
      <div className="flex h-[220px] items-center justify-center border-t border-[var(--border-subtle)] text-xs text-[var(--text-muted)] font-[320]">
  ...
  return (
    <div className="border-t border-[var(--border-subtle)] px-5 pb-4 pt-5">
      ...
      <div className="mt-1 flex items-center justify-between text-[10px] text-[var(--text-muted)] font-[320]">
...
export default function ArtistAnalytics({ artistId }: { artistId: string }) {
  ...
  if (error && !data) {
    return (
      <div className="filmwave-backend-section flex min-h-[320px] items-center justify-center px-6 text-center text-xs text-[var(--text-secondary)] font-[320]">
        {error}
      </div>
    );
  }
  ...
  return (
    <div className="grid gap-4">
      {error ? (
        <div className="filmwave-backend-section px-5 py-3 text-xs text-[var(--text-primary)] font-[320]">
          {error}
        </div>
      ) : null}
      ...
      <section className="filmwave-backend-section">
        ...
        {tracks.length === 0 ? (
          <div className="px-5 py-10 text-center text-xs text-[var(--text-muted)] font-[320]">
            No tracks to report yet.
          </div>
        ) : (
          <div className="overflow-x-auto">
            <div className="min-w-[720px]">
              <div className="grid grid-cols-[minmax(250px,1fr)_90px_90px_110px_110px] items-center border-b border-[var(--border-subtle)] px-5 py-3 text-[10px] text-[var(--text-secondary)] font-[320]">
                <span>Track</span>
              ...
              {tracks.map((track, index) => (
                ...
                >
                  <div className="flex min-w-0 items-center gap-3 pr-4">
                    <div className="flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden rounded-[6px] border border-[var(--border)] bg-[var(--bg-tertiary)] text-[11px] text-[var(--text-muted)] font-[320]">
                      {track.cover_url ? (
                        <img
                          src={track.cover_url}
                          alt=""
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="mt-1 text-[10px] text-[var(--text-muted)] font-[320]">
