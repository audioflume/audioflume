                  ...
                  <div className="flex min-h-10 min-w-0 cursor-pointer flex-wrap items-center gap-1 rounded-[7px] border border-[var(--border)] bg-[var(--bg-primary)] p-1">
                    {selectedDesignations.map((option) => (
                      <button
                        ...
                        className="inline-flex h-7 shrink-0 cursor-pointer items-center gap-1 rounded-[5px] bg-[var(--bg-tertiary)] px-2.5 text-[11px] text-[var(--text-secondary)] transition hover:text-[var(--text-primary)]"
                      >
                        <span>{option}</span>
            ...
            {Array.from({ length: TOTAL_STEPS }, (_, index) => {
              ...
              return (
                <span
                  key={indicatorStep}
                  aria-hidden="true"
                  className={`h-1 rounded-full bg-[var(--border)] transition-all duration-300 ease-out ${
                    active ? "w-6" : "w-1"
                  }`}
                />
              );
