...
export function CardTitle({ title, description }: { title: string; description?: string }) {
  return (
    <div className="border-b border-[var(--border)] px-4 py-3.5">
      <h2 className="text-2xl tracking-[-0.02em] text-[var(--text-primary)] font-[200]">{title}</h2>
      {description ? <p className="mt-1 text-xs leading-5 text-[var(--text-muted)] font-[320]">{description}</p> : null}
    </div>
  );
}
...
export function Input({ label, value, placeholder, onChange, readOnly = false, type = "text" }: { label: string; value?: string; placeholder?: string; onChange?: (value: string) => void; readOnly?: boolean; type?: string }) {
  return (
    <label className="block">
      <span className="mb-1.5 block text-xs text-[var(--text-muted)] font-[320]">{label}</span>
      <input
        type={type}
        value={value || ""}
...
export function Info({ label, value }: { label: string; value: string }) {
  return (
    <div className="border border-[var(--border)] bg-[var(--bg-primary)] px-3.5 py-3">
      <div className="text-xs text-[var(--text-muted)] font-[320]">{label}</div>
      <div className="mt-1 truncate text-sm text-[var(--text-primary)] font-[320]">{value}</div>
    </div>
  );
}
...
export function DangerButton({ children, onClick }: { children: React.ReactNode; onClick?: () => void }) {
  return (
    <button type="button" onClick={onClick} className="inline-flex h-8 cursor-pointer items-center justify-center border border-[var(--border)] bg-transparent px-3.5 text-xs text-[var(--danger)] transition hover:border-transparent hover:bg-[var(--danger-hover)] hover:text-[var(--danger)] font-[320]">
      {children}
    </button>
  );
}
  ...
  return (
    <div className="grid gap-4 border-b border-[var(--border)] px-4 py-4 last:border-b-0 md:grid-cols-[1fr_auto] md:items-center">
      <div className="flex gap-3">
        ...
        <div>
          <div className="text-sm tracking-[-0.02em] text-[var(--text-primary)] font-[320]">{title}</div>
          <p className="mt-1 max-w-xl text-xs leading-5 text-[var(--text-muted)] font-[320]">{description}</p>
        </div>
      </div>
  ...
  return (
    <section className="mb-8">
      <div className="mb-6 grid gap-6 xl:grid-cols-[minmax(0,0.9fr)_minmax(360px,0.72fr)] xl:items-end">
        <div>
          <div className="mb-3 inline-flex items-center text-[11px] uppercase tracking-[0.08em] text-[var(--text-muted)] font-[320]">{config.eyebrow}</div>
          <h1 className="max-w-[760px] font-[family-name:var(--font-zalando-sans)] text-[clamp(42px,6vw,76px)] leading-[0.9] tracking-[-0.07em] text-[var(--text-primary)] font-[400]">{config.title}</h1>
        </div>
        <p className="max-w-[520px] text-sm leading-6 text-[var(--text-secondary)] xl:justify-self-end font-[320]">{config.description}</p>
      </div>
      <div className="group relative min-h-[255px] overflow-hidden border border-[var(--border)] bg-[var(--bg-secondary)]">
        ...
        <div className="relative z-10 flex min-h-[255px] flex-col justify-between p-5 md:p-6">
          <div className="inline-flex w-fit max-w-full items-center rounded-full border border-white/15 bg-white/10 px-3 py-1 text-[11px] leading-none text-white/75 backdrop-blur font-[320]"><span className="truncate">Audioflume account</span></div>
          <div className="grid gap-2 sm:grid-cols-3">
            {[config.statA, config.statB, config.statC].map((stat) => <div key={stat} className="rounded-full border border-white/15 bg-white/10 px-3 py-2 text-[11px] text-white/80 backdrop-blur font-[320]">{stat}</div>)}
          </div>
        </div>
      </div>
    </section>
...
export function ProfileImageUploader({ initials, value, onChange }: { initials: string; value: string; onChange: (value: string) => void }) {
  ...
  return (
    <div className="flex flex-col items-start gap-2">
      <label className="group relative flex h-16 w-16 cursor-pointer items-center justify-center overflow-visible rounded-full border border-[var(--border)] bg-[var(--bg-primary)] text-sm text-[var(--text-primary)] transition hover:bg-[var(--bg-hover)] font-[320]">
      ...
      {value ? <button type="button" onClick={removeImage} className="text-[11px] text-[var(--text-muted)] transition hover:text-[var(--text-primary)] font-[320]">Remove photo</button> : null}
    </div>
  );
}
