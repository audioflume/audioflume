    ...
    <article className="admin-cover-hover group min-w-0">
      ...
      <div className="mt-2.5 flex min-w-0 items-start gap-3">
        <Link href={editHref} className="min-w-0 flex-1">
          <h3 className="truncate text-[13px] tracking-[-0.02em] text-[var(--text-primary)] font-[320]">
            {playlist.name}
          </h3>
...
export function PlaylistManagerCollapsibleSection({
  ...
  return (
    <section className="rounded-[10px] border border-[var(--border)] bg-[var(--bg-primary)] p-4 sm:p-5">
      <div
        className={`${wrapHeader ? "flex flex-wrap" : "flex"} items-center justify-between gap-4`}
      >
        <div>
          <h3 className="text-base font-medium tracking-[-0.03em] text-[var(--text-primary)]">
            {title}
          </h3>
          <p className="mt-1 text-[11px] text-[var(--text-muted)] font-[320]">
            {subtitle}
          </p>
        </div>
...
export function PlaylistManagerLibrarySection({
  ...
  return (
    <section className="mt-0">
      ...
      {playlists.length === 0 ? (
        <div className="flex min-h-[180px] items-center justify-center border border-[var(--border)] text-sm text-[var(--text-secondary)] font-[320]">
          {emptyMessage}
        </div>
      ) : (
