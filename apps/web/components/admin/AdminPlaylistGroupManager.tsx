    ...
    >
      <form id="playlist-group-form" onSubmit={handleSubmit} className="grid gap-4 pt-1">
        <label className="grid gap-1.5">
          <span className="text-[11px] text-[var(--text-secondary)] font-[320]">Group name</span>
          <BackendInput
            value={name}
        ...
        <label className="grid gap-1.5">
          <span className="text-[11px] text-[var(--text-secondary)] font-[320]">Description</span>
          <BackendTextarea
            value={description}
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1 py-2">
        <div className="truncate text-sm text-[var(--text-primary)] font-[320]">{group.name}</div>
        <div className="mt-0.5 text-[11px] text-[var(--text-muted)] font-[320]">
      ...
      <>
        <div className="overflow-hidden rounded-xl border border-[var(--border)] bg-[var(--bg-secondary)]">
          <div className="flex h-[58px] items-center justify-between border-b border-[var(--border)] px-4">
            <div>
              <h2 className="text-sm text-[var(--text-primary)] font-[320]">Playlist Groups</h2>
              <p className="mt-0.5 text-[11px] text-[var(--text-secondary)] font-[320]">
            ...
            <button
              ...
              className="flex items-center gap-1 text-xs text-[var(--text-secondary)] transition hover:text-[var(--text-primary)] font-[320]"
            >
              <PlusIcon />
              New
            </button>
          ...
          {!loading && error && <div className="px-4 py-3 text-sm text-[var(--danger)] font-[320]">{error}</div>}

          {!loading && !error && sortedGroups.length === 0 && (
            <div className="flex min-h-[100px] items-center justify-center px-4 text-sm text-[var(--text-secondary)] font-[320]">
              No groups yet.
            </div>
          )}
    ...
    <>
      <section className="rounded-2xl border border-[var(--border)] bg-[var(--bg-secondary)] p-5">
        <div className="mb-5 flex items-start justify-between gap-4">
          <div>
            <div className="text-[11px] uppercase tracking-[0.12em] text-[var(--text-muted)] font-[320]">Groups</div>
            <h2 className="mt-2 font-[family-name:var(--font-zalando-sans)] text-2xl tracking-[-0.05em] font-[200]">Playlist Groups</h2>
            <p className="mt-2 text-sm leading-6 text-[var(--text-secondary)] font-[320]">Create, rename, and delete the row headers used on the Curated Playlists page.</p>
          </div>
        ...
        {!loading && error && (
          <div className="rounded-xl border border-[var(--border)] bg-[var(--bg-primary)] p-4 text-sm text-[var(--danger)] font-[320]">{error}</div>
        )}

        {!loading && !error && sortedGroups.length === 0 && (
          <div className="rounded-xl border border-dashed border-[var(--border)] bg-[var(--bg-primary)] p-4 text-sm text-[var(--text-secondary)] font-[320]">No groups yet.</div>
        )}
              ...
              return (
                <div key={group.id} className="flex items-center gap-3 rounded-xl border border-[var(--border)] bg-[var(--bg-primary)] p-2">
                  <div className="min-w-0 flex-1">
                    <div className="truncate text-sm text-[var(--text-primary)] font-[320]">{group.name}</div>
                    <div className="mt-0.5 text-xs text-[var(--text-muted)] font-[320]">
                      {group.playlist_count || 0} playlist{group.playlist_count === 1 ? "" : "s"}
                      {isDefaultGroup ? " · Default" : ""}
                    </div>
                  </div>
