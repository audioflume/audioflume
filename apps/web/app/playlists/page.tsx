      ...
      <DropdownShell
        ...
        trigger={({ open }) => (
          <button
            ...
            className={
              viewMode === "grid"
                ? `playlist-menu-btn playlist-menu-btn-grid ${open ? "is-open" : ""}`
                : `playlist-menu-btn ${smallIconButtonClass} ${open ? `is-open ${iconButtonActiveClass}` : ""}`
            }
...
export default function PlaylistsPage() {
  ...
  return (
    <>
      <style>{`
        .playlists-page { position: relative; margin-left: var(--sidebar-width); margin-top: 56px; min-height: calc(100vh - 56px); overflow-x: hidden; overflow-y: visible; background: var(--bg-primary); color: var(--text-primary); transition: margin-left 0.2s ease; }
        .playlists-shell { position: relative; z-index: 1; padding: 0 32px; }
        .playlists-hero { display: block; padding: 88px 0 0; }
        .playlists-kicker { font-size: 10px; font-weight: 320; letter-spacing: 0.1em; text-transform: uppercase; color: var(--text-muted); }
        .playlists-title { margin-top: 8px; max-width: 640px; font-family: var(--font-zalando-sans); font-size: 56px; font-weight: 400; line-height: 0.94; letter-spacing: -0.055em; color: var(--text-primary); }
        .playlists-meta { margin-top: 16px; display: flex; flex-wrap: wrap; align-items: center; gap: 8px; font-size: 11px;
 font-weight: 320; color: var(--text-secondary); }
        ...
        .playlists-control-left { display: none; }
        .playlists-control-right { display: flex; align-items: center; gap: 8px; }
        .playlists-control-label { font-size: 10px; font-weight: 320; letter-spacing: 0.1em; text-transform: uppercase; color: var(--text-muted); }
        ...
        .playlist-edit-banner { min-height: 54px; display: flex; align-items: center; justify-content: space-between; gap: 16px; margin-top: 16px; margin-bottom: 24px; padding: 0 16px; border: 1px dashed var(--border); border-radius: 16px; background: var(--bg-secondary); }
        .playlist-edit-banner span { font-size: 13px;
 font-weight: 320; color: var(--text-secondary); }
        .playlist-edit-actions { display: flex; align-items: center; gap: 8px; }
        .playlist-edit-actions button { height: 30px; border-radius: 999px; padding: 0 15px; border: none; font-size: 12px; font-weight: 320; cursor: pointer; transition: opacity 0.15s ease, background 0.15s ease; }
        .playlist-edit-cancel { background: var(--bg-tertiary); color: var(--text-primary); }
        .playlist-edit-cancel:hover { background: var(--bg-hover); }
        .playlist-edit-save { background: var(--text-primary); color: var(--bg-primary); }
        .playlist-edit-save:hover { opacity: 0.82; }
        .playlist-library { padding-top: 0; }
        ...
        .playlist-gallery-art::after { content: ""; position: absolute; inset: 0; background: linear-gradient(to top, rgba(0, 0, 0, 0.62), rgba(0, 0, 0, 0.18) 58%, transparent); pointer-events: none; }
        ...
        .playlist-gallery-letters { position: absolute; left: 18px; bottom: 18px; z-index: 1; font-family: var(--font-zalando-sans); font-size: 44px; font-weight: 200; line-height: 0.9; letter-spacing: -0.07em; color: rgba(255, 255, 255, 0.12); pointer-events: none; }
        ...
        .playlist-gallery-kicker { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font-size: 10px; font-weight: 320; letter-spacing: 0.12em; text-transform: uppercase; color: rgba(255, 255, 255, 0.52); }
        .playlist-gallery-content h3 { margin-top: 8px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font-family: var(--font-zalando-sans); font-size: 25px; font-weight: 200; line-height: 1.15; letter-spacing: -0.055em; color: white; }
        .playlist-gallery-content p { margin-top: 12px; font-size: 11px; font-weight: 320; color: rgba(255, 255, 255, 0.58); }
        ...
        .playlist-gallery-rename-input { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font-family: var(--font-zalando-sans); font-size: 13px; font-weight: 320; line-height: 1.1; letter-spacing: -0.035em; }
        .playlist-row-rename-input { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font-size: 13.5px; font-weight: 320; line-height: 1.35; letter-spacing: normal; }
        ...
        .playlist-row-handle { width: 16px; display: flex; align-items: center; justify-content: center; color: var(--text-muted); }
        .playlist-row-number { font-size: 10px; font-weight: 320; color: var(--text-muted); letter-spacing: 0.05em; }
        .playlist-row-cover { position: relative; width: 50px; height: 50px; border-radius: 10px; overflow: hidden; background: var(--bg-secondary); }
        .playlist-row-main { min-width: 0; display: flex; flex-direction: column; gap: 5px; }
        .playlist-row-main span { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font-size: 14px; font-weight: 320; color: var(--text-primary); }
        .playlist-row-main small { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font-size: 11px;
 font-weight: 320; color: var(--text-muted); }
        .playlist-row-count { text-align: right; font-size: 11px;
 font-weight: 320; color: var(--text-secondary); }
        .playlist-card-menu-wrap { position: absolute; z-index: 12; top: 16px; left: 16px; }
        .playlist-index-row-shell .playlist-card-menu-wrap { top: 50%; left: auto; right: 12px; transform: translateY(-50%); }
        .playlist-menu-btn { opacity: 0; transition: opacity 0.15s ease, background-color 0.15s ease, color 0.15s ease, box-shadow 0.15s ease; }
        .playlist-menu-btn-grid { width: 32px; height: 32px; display: flex; align-items: center; justify-content: center; border: 1px solid rgba(255, 255, 255, 0.14); border-radius: 999px; background-color: rgba(255, 255, 255, 0.1); color: rgba(255, 255, 255, 0.72); cursor: pointer; backdrop-filter: blur(12px); }
        .playlist-gallery-card:hover .playlist-menu-btn-grid, .playlist-menu-btn-grid.is-open { opacity: 1; }
        .playlist-gallery-card:hover .playlist-menu-btn-grid:not(:hover):not(.is-open) { background-color: rgba(255, 255, 255, 0.1); color: rgba(255, 255, 255, 0.72); box-shadow: none; }
        .playlist-gallery-card [data-playlist-menu] .playlist-menu-btn-grid:hover, .playlist-gallery-card [data-playlist-menu] .playlist-menu-btn-grid.is-open { background-color: white; color: black; }
        .playlist-index-row-shell .playlist-menu-btn { background: transparent; color: var(--icon-color); backdrop-filter: none; }
        .playlist-index-row-shell:hover .playlist-menu-btn, .playlist-index-row-shell .playlist-menu-btn.is-open { opacity: 1; }
        .playlist-index-row-shell .playlist-menu-btn:hover, .playlist-index-row-shell .playlist-menu-btn.is-open { background: var(--icon-button-hover); color: var(--text-primary); }
        .playlist-create-card { position: relative; min-height: 210px; width: 100%; display: flex; flex-direction: column; justify-content: flex-end; gap: 12px; border-radius: 18px; border: 1px dashed var(--border); background: var(--bg-card); padding: 16px; cursor: pointer; text-align: left; transition: background 0.15s ease, border-color 0.15s ease; }
        .playlist-create-card:hover { border-color: var(--border-hover); background: var(--bg-hover); }
        .playlist-create-row { grid-template-columns: 34px minmax(0, 1fr) 34px; padding: 11px 46px 11px 18px; border: 1px dashed var(--border); background: var(--bg-card); }
        ...
        .playlist-create-copy span { font-size: 14px; font-weight: 320; color: var(--text-primary); }
        .playlist-create-copy small { font-size: 11px;
 font-weight: 320; color: var(--text-secondary); }
        .playlist-deleting-overlay { position: absolute; inset: 0; z-index: 30; display: flex; align-items: center; justify-content: center; border-radius: inherit; background: var(--media-overlay-strong); pointer-events: none; }
        .drag-preview-card { width: 320px; overflow: hidden; border-radius: 18px; transform: scale(1.04); box-shadow: 0 24px 80px var(--media-overlay-heavy); }
        .drag-preview-card .playlist-gallery-art-wrap { overflow: hidden; border-radius: 18px; }
        .drag-preview-row { width: calc(100vw - var(--sidebar-width) - 64px); box-shadow: 0 24px 80px var(--media-overlay-heavy); }
        .playlist-skeleton-reserve { min-height: 280px; }
        .playlist-skeleton-block { position: relative; overflow: hidden; background: var(--bg-tertiary); }
        .playlist-skeleton-block::after { content: ""; position: absolute; inset: 0; transform: translateX(-100%); background: linear-gradient(90deg, transparent, color-mix(in srgb, var(--bg-hover) 72%, transparent), transparent); animation: playlist-skeleton-shimmer 1.6s ease-in-out infinite; }
        @keyframes playlist-skeleton-shimmer { 100% { transform: translateX(100%); } }
        .playlist-skeleton-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); gap: 16px; }
        .playlist-skeleton-gallery-card { min-width: 0; animation: skeleton-fade-in 0.3s ease-out both; }
        .playlist-skeleton-gallery-art { position: relative; min-height: 210px; border-radius: 18px; border: 1px solid var(--border-subtle); background: var(--bg-card); overflow: hidden; }
        .playlist-skeleton-gallery-art::after { content: ""; position: absolute; inset: 0; transform: translateX(-100%); background: linear-gradient(90deg, transparent, color-mix(in srgb, var(--bg-hover) 48%, transparent), transparent); animation: playlist-skeleton-shimmer 1.6s ease-in-out infinite; }
        .playlist-skeleton-gallery-arrow { position: absolute; right: 16px; top: 16px; width: 32px; height: 32px; border-radius: 999px; background: var(--bg-tertiary); }
        .playlist-skeleton-list { display: flex; flex-direction: column; gap: 8px; }
        .playlist-skeleton-index-row { position: relative; min-height: 76px; display: grid; grid-template-columns: 40px 50px minmax(0, 1fr) minmax(84px, 120px) 28px; gap: 14px; align-items: center; border: 1px solid var(--border-subtle); border-radius: 16px; background: transparent; padding: 11px 13px; animation: skeleton-fade-in 0.3s ease-out both; }
        .playlist-skeleton-number { width: 22px; height: 8px; }
        .playlist-skeleton-row-cover { width: 50px; height: 50px; border-radius: 10px; }
        .playlist-skeleton-row-copy { display: flex; min-width: 0; flex-direction: column; gap: 9px; }
        .playlist-skeleton-row-title { width: min(220px, 58%); height: 9px; }
        .playlist-skeleton-row-meta { width: min(300px, 42%); height: 8px; }
        .playlist-skeleton-row-count { justify-self: end; width: 58px; height: 8px; }
        .playlist-skeleton-row-menu { justify-self: end; width: 28px; height: 28px; }
        @media (max-width: 980px) { .playlists-control-bar { margin-top: -34px; } }
        @media (max-width: 720px) {
          .playlists-hero { padding-top: 88px; }
          .playlists-control-bar { margin-top: -34px; margin-bottom: 32px; }
          .playlist-gallery, .playlist-skeleton-grid { grid-template-columns: repeat(auto-fill, minmax(240px, 1fr)); gap: 14px; }
          .playlist-gallery-art-wrap, .playlist-create-card, .playlist-skeleton-gallery-art { min-height: 188px; }
          .playlist-gallery-content h3 { font-size: 23px;
 font-weight: 200; }
          .drag-preview-row { width: calc(100vw - var(--sidebar-width) - 36px); }
        }
        @media (max-width: 520px) {
          .playlist-index-row { grid-template-columns: 18px 50px minmax(0, 1fr) minmax(70px, 96px); gap: 12px; padding: 11px 46px 11px 13px; }
          .playlist-index-row.is-reordering { grid-template-columns: 16px 34px 50px minmax(0, 1fr) minmax(70px, 96px); gap: 12px; padding-right: 18px; }
          .playlist-create-row { grid-template-columns: 50px minmax(0, 1fr) 50px; padding: 11px 18px; }
          .playlist-create-row .playlist-create-mark { grid-column: 1; }
          .playlist-create-row .playlist-create-copy { grid-column: 2; align-items: center; text-align: center; }
          .playlist-create-row .playlist-create-copy span, .playlist-create-row .playlist-create-copy small { white-space: normal; }
          .playlist-skeleton-index-row { grid-template-columns: 18px 50px minmax(0, 1fr) minmax(70px, 96px) 28px; gap: 12px; padding: 11px 13px; }
          .playlist-skeleton-row-title { width: 68%; }
          .playlist-skeleton-row-meta { width: 50%; }
        }
      `}</style>
