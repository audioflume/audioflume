...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    const { data: releaseLink, error: linkError } = await supabaseServer
    ...
    const key = `images/artists/${id}/releases/${releaseId}/${Date.now()}-${randomUUID()}.webp`;
    ...
    console.error("Artist release artwork upload failed:", error);
    return NextResponse.json(
      {
        error:
          error instanceof Error
            ? error.message
            : "Failed to upload release artwork",
      },
      { status: 500 },
    );
  }
