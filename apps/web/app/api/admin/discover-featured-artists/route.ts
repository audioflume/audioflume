...
function featuredArtistError(error: unknown) {
  const message =
    error && typeof error === "object" && "message" in error
      ? String((error as { message?: unknown }).message || "")
      : "";

  if (message.toLowerCase().includes("discover_featured_artists")) {
    return "Featured artists require the latest Discover migration.";
  }

  return message || "Failed to update featured artists";
}
...
export async function GET() {
  ...
  try {
    ...
    const { data: artists, error: artistsError } = await supabaseServer
      .from("artists")
      .select(
        "id, name, slug, profile_image_url, hero_image_url, status",
      )
      .in("id", artistIds);
    ...
    return NextResponse.json({
      items: (featuredRows ?? []).flatMap((row) => {
    ...
    console.error("Admin featured artists fetch failed:", error);
    return NextResponse.json(
      { error: featuredArtistError(error) },
      { status: 500 },
    );
  }
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (eligibleIds.length === 0) {
      return NextResponse.json(
        { error: "Only approved artists can be featured" },
        { status: 400 },
      );
    }
    ...
    console.error("Admin featured artist add failed:", error);
    return NextResponse.json(
      { error: featuredArtistError(error) },
      { status: 500 },
    );
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    console.error("Admin featured artist reorder failed:", error);
    return NextResponse.json(
      { error: featuredArtistError(error) },
      { status: 500 },
    );
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    console.error("Admin featured artist remove failed:", error);
    return NextResponse.json(
      { error: featuredArtistError(error) },
      { status: 500 },
    );
