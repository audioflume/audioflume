...
function TagGroup({ label, values }: { label: string; values: string[] }) {
  ...
  return (
    <div>
      <div className="mb-2 text-[10px] uppercase tracking-[0.05em] text-[var(--text-muted)] font-[320]">
        {label}
      </div>
      <div className="flex flex-wrap gap-1.5">
        {values.map((value) => (
          <span
            key={value}
            className="inline-flex h-7 items-center rounded-full bg-[var(--bg-tertiary)] px-2.5 text-[11px] text-[var(--text-secondary)] font-[320]"
          >
            {value}
          </span>
        ))}
...
function DetailValue({ label, value }: { label: string; value: string }) {
  return (
    <div>
      <div className="text-[10px] uppercase tracking-[0.05em] text-[var(--text-muted)] font-[320]">
        {label}
      </div>
      <div className="mt-1.5 text-xs text-[var(--text-primary)] font-[320]">{value || "—"}</div>
    </div>
  );
}
    ...
    >
      {selectedSongId ? (
        <div className="grid gap-4">
          ...
          {detailsLoading ? (
            <div className="filmwave-backend-section flex min-h-[320px] items-center justify-center text-xs text-[var(--text-muted)] font-[320]">
              Loading submission...
            </div>
          ) : song ? (
            <>
              ...
              <section className="filmwave-backend-section">
                ...
                <div className="p-5">
                  {credits.length === 0 ? (
                    <div className="text-xs text-[var(--text-muted)] font-[320]">No credits supplied.</div>
                  ) : (
              ...
              <section className="filmwave-backend-section">
                <div className="filmwave-backend-section-header-bordered">
                  ...
                  <span className="text-[10px] uppercase tracking-[0.05em] text-[var(--text-muted)] font-[320]">
                ...
                <div className="grid gap-5 p-5">
                  ...
                  <div className="overflow-x-auto rounded-[7px] border border-[var(--border)] bg-[var(--bg-primary)]">
                    <div className="grid min-w-[760px] grid-cols-[minmax(180px,1fr)_150px_100px_150px_150px] gap-3 border-b border-[var(--border-subtle)] px-4 py-3 text-[10px] uppercase tracking-[0.05em] text-[var(--text-muted)] font-[320]">
                    ...
                    {rightsHolders.length === 0 ? (
                      <div className="px-4 py-4 text-xs text-[var(--text-muted)] font-[320]">
                      ...
                      rightsHolders.map((holder, index) => (
                        <div
                          ...
                          className="grid min-w-[760px] grid-cols-[minmax(180px,1fr)_150px_100px_150px_150px] gap-3 border-b border-[var(--border-subtle)] px-4 py-3 text-xs last:border-b-0 font-[320]"
                        >
                  ...
                  {rights?.notes ? (
                    <div className="rounded-[7px] bg-[var(--bg-secondary)] px-4 py-3 text-xs leading-5 text-[var(--text-secondary)] font-[320]">
                      {rights.notes}
                    </div>
                  ) : null}
                </div>
                    ...
                    {reviews.map((review) => (
                      <div key={review.id} className="px-5 py-4">
                        <div className="flex flex-wrap items-center justify-between gap-3">
                          ...
                          <span className="text-[11px] text-[var(--text-muted)] font-[320]">
                            {formatDate(review.created_at)}
                          </span>
                        </div>
                        {review.notes ? (
                          <p className="mt-2 text-xs leading-5 text-[var(--text-secondary)] font-[320]">
                            {review.notes}
                          </p>
                        ) : null}
                      </div>
              ...
              {song.status === "submitted" || song.status === "approved" ? (
                <section className="filmwave-backend-section p-5">
                  <label className="block">
                    <span className="mb-1.5 block text-[10px] uppercase tracking-[0.05em] text-[var(--text-muted)] font-[320]">
          ...
          <section className="filmwave-backend-section">
            <div className="flex items-center justify-between gap-4 px-5 py-5">
              <div>
                ...
                <div className="mt-1 text-xs text-[var(--text-secondary)] font-[320]">
            ...
            <div className="overflow-x-auto overflow-y-hidden border-t border-[var(--border-subtle)]">
              <div className="min-w-[980px]">
                {loading ? (
                  <div className="flex min-h-[180px] items-center justify-center px-5 text-xs text-[var(--text-muted)] font-[320]">
                    Loading submissions...
                  </div>
                ) : error ? (
                  <div className="flex min-h-[180px] flex-col items-center justify-center gap-3 px-5 text-center text-xs text-[var(--text-secondary)] font-[320]">
                    <span>{error}</span>
                    <button
                      type="button"
                  ...
                  <div>
                    {visibleSongs.map((item, index) => (
                      <div
                        key={item.id}
                        className="grid min-h-[72px] grid-cols-[60px_minmax(220px,1.6fr)_76px_90px_76px_110px_124px_minmax(220px,auto)] items-center gap-4 px-5 text-xs transition hover:bg-[var(--bg-hover)] font-[320]"
                        style={{
                          borderBottom:
