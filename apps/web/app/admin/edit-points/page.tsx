...
function ResultStatus({ status }: { status: BatchAnalyzeResult["status"] }) {
  return (
    <div className="flex items-center gap-2 text-xs text-[var(--text-secondary)] font-[320]">
      <span
...
function SummaryRow({
  ...
  return (
    <div className="flex min-h-10 items-center justify-between gap-3 rounded-[7px] border border-[var(--border)] bg-[var(--bg-secondary)] px-3 text-xs text-[var(--text-secondary)] font-[320]">
      <span>{label}</span>
    ...
    >
      <div className="mb-4 flex flex-wrap items-center gap-2">
        <button
          ...
          className="inline-flex h-10 min-w-[104px] cursor-pointer items-center justify-center gap-2 rounded-[7px] border border-[var(--text-primary)] bg-[var(--text-primary)] px-5 text-[12px] text-[var(--bg-primary)] transition disabled:cursor-default disabled:opacity-50 font-[320]"
        >
          {isAnalyzing && (
        ...
        <Link
          ...
          className="inline-flex h-10 min-w-[104px] items-center justify-center rounded-[7px] border border-[var(--border)] bg-[var(--bg-secondary)] px-5 text-[12px] text-[var(--text-secondary)] transition hover:text-[var(--text-primary)] font-[320]"
        >
          View Missing
        </Link>
      </div>
          ...
          {result && (
            <section className="overflow-hidden rounded-[10px] border border-[var(--border)] bg-[var(--bg-primary)]">
              ...
              {result.results.length === 0 ? (
                <div className="flex min-h-[120px] items-center justify-center border-t border-[var(--border-subtle)] px-5 text-xs text-[var(--text-secondary)] font-[320]">
                  ...
                  {result.results.map((item, index) => (
                    ...
                    >
                      <div className="min-w-0">
                        ...
                        {item.error && (
                          <div className="mt-1 truncate text-[11px] text-[var(--status-error,#dc584f)] font-[320]">
                            {item.error}
                          </div>
                        )}
                      </div>
                      ...
                      <Link
                        href={`/admin/songs/${item.songId}/edit-points?from=edit-points`}
                        className="inline-flex h-8 items-center justify-center rounded-[7px] border border-[var(--border)] px-3 text-[12px] text-[var(--text-secondary)] transition hover:text-[var(--text-primary)] font-[320]"
                      >
                        Review
                      </Link>
                    </div>
