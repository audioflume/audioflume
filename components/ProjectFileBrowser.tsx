  ...
  return (
    <style>{`
      ...
      .project-preview-button {
        --project-preview-track: rgba(255, 255, 255, 0.18);
        position: absolute;
        inset: 0;
        margin: auto;
        display: flex;
      ...
      .light .project-preview-button {
        --project-preview-track: rgba(0, 0, 0, 0.18);
      }
...
function FolderGlyph({ small = false }: { small?: boolean }) {
  return (
    <span
      className={small ? "project-folder-glyph small" : "project-folder-glyph"}
    >
...
function MusicGlyph({ small = false }: { small?: boolean }) {
  return (
    <span
      className={small ? "project-music-glyph small" : "project-music-glyph"}
    >
      ♪
    </span>
  );
  ...
  if (viewMode === "list") {
    return (
      ...
      >
        ...
        <span className="project-browser-row-muted">
          {getAssetTypeLabel(folder.asset_type)}
        </span>
        <span />
      </button>
    );
...
function SongFileCard({
  ...
  const { currentSong, isPlaying, currentTime, duration, togglePlayPause } =
    usePlayer();
      ...
      <div className="project-browser-row project-file-row">
        <span className="project-browser-row-name">
          <button
            ...
            style={{
              background: `conic-gradient(var(--text-primary) ${progressDegrees}, var(--project-preview-track) 0deg)`,
            }}
            onClick={() => togglePlayPause(song)}
            aria-label={
              previewIsPlaying ? `Pause ${song.title}` : `Preview ${song.title}`
            }
          >
...
export default function ProjectFileBrowser({
  ...
  useEffect(() => {
    ...
    if (!wrap || document.querySelector(".project-google-drive-trigger"))
      return;
  ...
  const visibleSongs = useMemo(
    () =>
      songs.filter(
        (song) => (song.project_folder_id ?? null) === activeFolderId,
      ),
