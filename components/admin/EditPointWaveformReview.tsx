...
export default function EditPointWaveformReview({
  ...
  useEffect(() => {
    const onPointerMove = (event: PointerEvent) => {
      ...
      if (dragState.mode === "marker" && dragState.markerId) {
        updateMarkerTime(dragState.markerId, nextTime);
      }
    };
  ...
  const updateMarkerTime = (markerId: string, time: number) => {
    const nextTime = clampTime(time, effectiveDuration);
  ...
  const nudgeMarker = (markerId: string, amount: number) => {
    ...
    updateMarkerTime(markerId, marker.time + amount);
  };
    ...
    <div className="rounded-xl border border-[var(--border)] bg-[var(--bg-primary)] p-4">
      ...
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-[var(--text-secondary)]">
            Spacebar toggles playback. Drag the playhead or edit markers, then save corrected points.
          </p>
        </div>
      ...
      <div className="grid gap-3 md:grid-cols-[42px_minmax(0,1fr)] md:items-center">
        <button
          ...
          className="flex h-10 w-10 items-center justify-center rounded-full border border-[var(--border)] bg-[var(--text-primary)] text-[var(--bg-primary)] transition hover:scale-[1.03] hover:opacity-90 disabled:cursor-default disabled:opacity-40 disabled:hover:scale-100"
        >
            ...
            return (
              <button
                ...
                onPointerDown={(event) => {
                  ...
                  dragStateRef.current = { mode: "marker", markerId: marker.id };
              ...
              >
                ...
                <span className="sr-only">
                  Drag {marker.label} at {formatTime(marker.time)}
                </span>
              </button>
            );
          })}
      ...
      <div className="mt-4 overflow-hidden rounded-xl border border-[var(--border)]">
        <div className="grid grid-cols-[minmax(0,1.2fr)_120px_130px] border-b border-[var(--border)] bg-[var(--bg-secondary)] px-3 py-2 text-[11px] font-medium uppercase tracking-[0.08em] text-[var(--text-muted)]">
          <div>Marker</div>
          <div>Time</div>
          <div>Nudge</div>
        </div>
        ...
        {localMarkers.map((marker) => {
          ...
          return (
            <div
              key={marker.id}
              className={`grid grid-cols-[minmax(0,1.2fr)_120px_130px] items-center border-b border-[var(--border-subtle)] px-3 py-2 text-xs last:border-b-0 ${
            ...
            >
              ...
              >
                ...
                <div className="mt-1 truncate text-[11px] text-[var(--text-muted)]">
                  {getConfidenceLabel(marker.confidence)} · {Math.round(marker.confidence * 100)}% · {marker.source}
                </div>
              </button>
              ...
              <input
                ...
                onChange={(event) => {
                  ...
                  updateMarkerTime(marker.id, parsed);
                }}
