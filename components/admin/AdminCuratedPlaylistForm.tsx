...
export default function AdminCuratedPlaylistForm({ mode, playlistId }: Props) {
  ...
  useEffect(() => {
    ...
    async function loadPlaylist() {
      try {
        ...
        if (!cancelled) setToastMessage(err instanceof Error ? err.message : "Failed to load playlist");
      } finally {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const res = await fetch(endpoint, {
        ...
        body: JSON.stringify({ name, kicker, cover_image_url: coverImageUrl, playlist_group: playlistGroup, show_on_discover: showOnDiscover }),
      });
      ...
      if (mode === "create") router.push(`/admin/playlist-manager/${data.id}/edit`);
    } catch (err) {
  ...
  // slug derived from name for the upload key
    ...
    <>
      <form onSubmit={handleSubmit} className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
        <section className="rounded-2xl border border-[var(--border)] bg-[var(--bg-secondary)] p-5">
          <div className="mb-5">
            <h2 className="font-[family-name:var(--font-instrument-sans)] text-2xl font-medium tracking-[-0.05em]">Playlist details</h2>
            <p className="mt-1 text-sm text-[var(--text-secondary)]">Add the public metadata used on Curated Playlists cards and rows.</p>
          </div>
          ...
          {loading ? (
            ...
            <div className="grid gap-4">
              <label className="grid gap-2 text-xs font-medium text-[var(--text-secondary)]">
                Playlist name
                <input value={name} onChange={(e) => setName(e.target.value)} className="h-11 rounded-lg border border-[var(--border)] bg-[var(--bg-primary)] px-3 text-sm text-[var(--text-primary)] outline-none transition focus:border-[var(--text-muted)]" placeholder="Docu beds" required />
              </label>

              <label className="grid gap-2 text-xs font-medium text-[var(--text-secondary)]">
                Kicker text
                <input value={kicker} onChange={(e) => setKicker(e.target.value)} className="h-11 rounded-lg border border-[var(--border)] bg-[var(--bg-primary)] px-3 text-sm text-[var(--text-primary)] outline-none transition focus:border-[var(--text-muted)]" placeholder="Human stories" />
              </label>

              <label className="grid gap-2 text-xs font-medium text-[var(--text-secondary)]">
                Playlist group
                <select value={playlistGroup} onChange={(e) => setPlaylistGroup(e.target.value)} className="h-11 rounded-lg border border-[var(--border)] bg-[var(--bg-primary)] px-3 text-sm text-[var(--text-primary)] outline-none transition focus:border-[var(--text-muted)]">
                  {CURATED_PLAYLIST_GROUPS.map((group) => <option key={group} value={group}>{group}</option>)}
                </select>
              </label>
              ...
              <label className="flex cursor-pointer items-start gap-3 rounded-xl border border-[var(--border)] bg-[var(--bg-primary)] p-3 text-sm text-[var(--text-secondary)]">
                <input type="checkbox" checked={showOnDiscover} onChange={(e) => setShowOnDiscover(e.target.checked)} className="mt-0.5 h-4 w-4 accent-[var(--text-primary)]" />
                <span>
              ...
              <div className="flex flex-wrap gap-3 pt-2">
                ...
                <button type="button" className={secondaryPillButtonClass} onClick={() => router.push("/admin/playlist-manager")}>
                  Back to manager
                </button>
              </div>
            </div>
          ...
          <div className="rounded-2xl border border-[var(--border)] bg-[var(--bg-secondary)] p-5">
            ...
            <div className="relative mt-4 min-h-[260px] overflow-hidden rounded-[18px] border border-[var(--border)] bg-[var(--bg-tertiary)]">
              ...
              <div className="relative z-10 flex min-h-[260px] flex-col justify-end p-4 text-white">
                ...
                <div className="mt-2 font-[family-name:var(--font-instrument-sans)] text-3xl font-medium leading-none tracking-[-0.055em]">{name || "Playlist name"}</div>
                <div className="mt-3 text-[11px] font-medium text-white/58">{playlistGroup}</div>
              </div>
            </div>
          </div>
        </aside>
        ...
        <section className="mt-6 rounded-2xl border border-[var(--border)] bg-[var(--bg-secondary)] p-5">
          <div className="mb-4 flex items-end justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-[var(--text-secondary)]">Add songs from the admin music player dropdown. Remove songs here while editing.</p>
            </div>
