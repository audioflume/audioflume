...
function Header({ eyebrow, title, description }: { eyebrow: string; title: string; description: string }) {
  return (
    <div className="mb-7 max-w-3xl">
      <div className="text-[10px] font-medium uppercase tracking-[0.24em] text-[var(--text-muted)]">{eyebrow}</div>
      <h1 className="mt-3 text-[34px] font-medium leading-none tracking-[-0.055em] text-[var(--text-primary)] md:text-[46px]">{title}</h1>
...
function Card({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return <div className={`overflow-hidden rounded-[18px] border border-[var(--border-subtle)] bg-[var(--bg-card)] ${className}`}>{children}</div>;
}
...
function Input({ label, value, placeholder }: { label: string; value?: string; placeholder?: string }) {
  return (
    <label className="block">
      <span className="mb-1.5 block text-[10px] font-medium uppercase tracking-[0.18em] text-[var(--text-muted)]">{label}</span>
      <input value={value || ""} placeholder={placeholder} readOnly className="h-10 w-full rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-primary)] px-3 text-sm text-[var(--text-primary)] outline-none placeholder:text-[var(--text-muted)]" />
    </label>
  );
}
...
function Info({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-primary)] px-3.5 py-3">
      <div className="text-[10px] font-medium uppercase tracking-[0.18em] text-[var(--text-muted)]">{label}</div>
...
function Button({ children, subtle = false }: { children: React.ReactNode; subtle?: boolean }) {
  return (
    <button type="button" className={`h-8 cursor-pointer rounded-lg border px-3.5 text-xs font-medium transition ${subtle ? "border-[var(--border-subtle)] bg-transparent text-[var(--text-secondary)] hover:bg-[var(--bg-hover)] hover:text-[var(--text-primary)]" : "border-[var(--border)] bg-[var(--bg-primary)] text-[var(--text-primary)] hover:bg-[var(--bg-hover-strong)]"}`}>
      {children}
    </button>
  );
}
...
function Option<T extends string>({ label, active, onClick }: { label: string; active: boolean; onClick: (value: T) => void }) {
  return (
    <button type="button" onClick={() => onClick(label.toLowerCase() as T)} className={`flex h-8 cursor-pointer items-center justify-center gap-2 rounded-lg border px-3 text-xs font-medium transition ${active ? "border-[var(--border)] bg-[var(--bg-hover-strong)] text-[var(--text-primary)]" : "border-[var(--border-subtle)] bg-transparent text-[var(--text-secondary)] hover:bg-[var(--bg-hover)] hover:text-[var(--text-primary)]"}`}>
    ...
    <>
      ...
      <div className="grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
        <Card>
          ...
          <div className="flex flex-wrap gap-2 border-t border-[var(--border-subtle)] px-4 py-3.5"><Button>Save changes</Button><Button subtle>Cancel</Button></div>
        </Card>
        <Card>
          <div className="border-b border-[var(--border-subtle)] p-4">
            <div className="flex items-center gap-3">
              <div className="flex h-12 w-12 items-center justify-center rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-primary)] text-sm font-medium text-[var(--text-primary)]">{initials || "FW"}</div>
              <div className="min-w-0"><div className="truncate text-base font-medium tracking-[-0.03em] text-[var(--text-primary)]">{fullName}</div><div className="mt-1 truncate text-xs text-[var(--text-muted)]">{email}</div></div>
            </div>
          </div>
          <div className="grid gap-3 p-4"><Info label="Account type" value="Lifetime Member" /><Info label="Library license" value="Royalty-free commercial use" /><Info label="Member since" value="Mock data" /></div>
        </Card>
      </div>
    </>
  );
  ...
  return (
    <>
      ...
      <Card>
        ...
        <Row title="Theme" description="Controls the light or dark appearance across Filmwave."><Option<ThemeMode> label="Dark" active={theme === "dark"} onClick={() => setTheme("dark")} /><Option<ThemeMode> label="Light" active={theme === "light"} onClick={() => setTheme("light")} /></Row>
        <Row title="Playlist view" description="Sets the default layout for your personal playlist library."><Option<PlaylistViewMode> label="Grid" active={playlistViewMode === "grid"} onClick={() => setPlaylistViewMode("grid")} /><Option<PlaylistViewMode> label="List" active={playlistViewMode === "list"} onClick={() => setPlaylistViewMode("list")} /></Row>
        <Row title="Playlist sorting" description="Choose whether playlists use your custom drag order or stay alphabetical."><Option<PlaylistSortMode> label="Custom" active={playlistSortMode === "custom"} onClick={() => setPlaylistSortMode("custom")} /><Option<PlaylistSortMode> label="Alphabetical" active={playlistSortMode === "alphabetical"} onClick={() => setPlaylistSortMode("alphabetical")} /></Row>
  ...
  return (
    <>
      ...
      <div className="grid gap-4 xl:grid-cols-[1.1fr_0.9fr]">
        <Card>
          <div className="border-b border-[var(--border-subtle)] p-4"><div className="text-[10px] font-medium uppercase tracking-[0.2em] text-[var(--text-muted)]">Current plan</div><div className="mt-2 text-2xl font-medium tracking-[-0.05em] text-[var(--text-primary)]">Lifetime Membership</div><p className="mt-2 max-w-xl text-sm leading-6 text-[var(--text-secondary)]">Unlimited access to the Filmwave library, curated playlists, waveform previews, playlist tools, and commercial project licensing.</p></div>
  ...
  return (
    <>
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_0.9fr]">
        <Card><CardTitle title="Payment method" description="Primary billing method used for renewals and plan changes." /><div className="p-4"><div className="rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-primary)] p-3.5"><div className="flex flex-wrap items-center justify-between gap-4"><div><div className="text-sm font-medium text-[var(--text-primary)]">Visa ending in 4242</div><div className="mt-1 text-xs text-[var(--text-muted)]">Expires 08/29 · Billing address in Canada</div></div><div className="rounded-md border border-[var(--border-subtle)] px-2 py-1 text-[11px] font-medium text-[var(--text-muted)]">Mock card</div></div></div><div className="mt-4 flex flex-wrap gap-2"><Button>Update payment method</Button><Button subtle>Add backup card</Button></div></div></Card>
      ...
      <Card className="mt-4"><CardTitle title="Recent invoices" description="Download receipts and track billing history." /><div className="divide-y divide-[var(--border-subtle)]">{["May 2026", "April 2026", "March 2026"].map((invoice) => <div key={invoice} className="flex flex-wrap items-center justify-between gap-3 px-4 py-3.5"><div><div className="text-sm font-medium text-[var(--text-primary)]">{invoice} invoice</div><div className="mt-1 text-xs text-[var(--text-muted)]">Paid · $15.00 CAD</div></div><Button subtle>Download</Button></div>)}</div></Card>
    </>
  );
}
...
function Support() {
  const cards = [["Submit a ticket", "Send a detailed issue report for billing, song files, downloads, licensing, or account problems.", "Start ticket"], ["Contact support", "Reach the Filmwave team directly for account questions or help using the library.", "Email support"], ["License help", "Find answers about commercial usage, client projects, social ads, and broadcast-style work.", "View license guide"]];
  ...
  return (
    <>
      ...
      <div className="grid gap-4 lg:grid-cols-3">{cards.map(([title, desc, action]) => <Card key={title} className="p-4"><div className="text-sm font-medium text-[var(--text-primary)]">{title}</div><p className="mt-2 min-h-[66px] text-xs leading-5 text-[var(--text-muted)]">{desc}</p><div className="mt-4"><Button subtle>{action}</Button></div></Card>)}</div>
      <Card className="mt-4"><CardTitle title="Frequently asked questions" /><div className="divide-y divide-[var(--border-subtle)]">{faqs.map(([q, a]) => <div key={q} className="px-4 py-3.5"><div className="text-sm font-medium text-[var(--text-primary)]">{q}</div><p className="mt-2 text-xs leading-5 text-[var(--text-muted)]">{a}</p></div>)}</div></Card>
    </>
  );
}
