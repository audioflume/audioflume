  ...
  return (
    <main className="min-h-screen bg-[var(--bg-primary)] pt-14 text-[var(--text-primary)] md:ml-[var(--admin-sidebar-width)]">
      ...
      <div className="px-5 py-6 md:px-8 lg:px-10">
        <section className="mb-6 flex flex-col gap-4 rounded-2xl border border-[var(--border)] bg-[var(--bg-secondary)] p-5 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-3 max-w-[620px] text-sm leading-6 text-[var(--text-secondary)]">
              Create and edit the curated playlists shown on the public Curated Playlists page.
            </p>
          </div>
        ...
        {loading && <div className="h-64 animate-pulse rounded-2xl bg-[var(--bg-secondary)]" />}

        {!loading && error && (
          <div className="rounded-2xl border border-[var(--border)] bg-[var(--bg-secondary)] p-6 text-sm text-[var(--danger)]">
            {error}
          </div>
        )}

        {!loading && !error && playlists.length === 0 && (
          <div className="rounded-2xl border border-dashed border-[var(--border)] bg-[var(--bg-secondary)] p-8 text-sm text-[var(--text-secondary)]">
          ...
          <div className="grid gap-3">
            {playlists.map((playlist) => (
              <Link
                ...
                className="group flex items-center gap-4 rounded-2xl border border-[var(--border)] bg-[var(--bg-secondary)] p-3 transition hover:border-[var(--text-muted)] hover:bg-[var(--bg-hover)]"
              >
                <div className="relative h-20 w-20 shrink-0 overflow-hidden rounded-xl bg-[var(--bg-tertiary)]">
                  {playlist.cover_image_url && (
                    <Image src={playlist.cover_image_url} alt={playlist.name} fill sizes="80px" className="object-cover" unoptimized />
                  )}
                </div>
