...
export type EditableSongLike = {
  editPoints?: string | FilmwaveEditPoints | null;
...
export function isCoreEditPointType(type: string) {
  return CORE_EDIT_POINT_TYPES.includes(
    normalizeEditPointType(type) as typeof CORE_EDIT_POINT_TYPES[number],
  );
}
...
function normalizeMarker(marker: unknown): FilmwaveEditPointMarker | null {
  ...
  return {
    id: String(record.id || `${record.type || record.label || "marker"}-${time}`),
...
export function parseEditPoints(value: EditableSongLike["editPoints"]): FilmwaveEditPoints {
  if (!value) {
...
export function getMarkerType(marker: FilmwaveEditPointMarker) {
...
export function getSongEditPointMarkers(song: EditableSongLike) {
  return parseEditPoints(song.editPoints).markers || [];
}

export function getSongCuePointMarkers(song: EditableSongLike) {
...
export function songHasEditPointType(song: EditableSongLike, type: string) {
...
function getMarkerTime(song: EditableSongLike, type: string) {
  const normalizedType = normalizeEditPointType(type);
...
function getSongDuration(song: EditableSongLike) {
  const duration = Number(song.duration ?? song.durationSeconds ?? 0);
  return Number.isFinite(duration) ? duration : 0;
}

export function songMatchesEditPointFilter(song: EditableSongLike, type: string) {
  const normalizedType = normalizeEditPointType(type);
  ...
  if (normalizedType === "break_after_midpoint") {
    return breakPoint !== null && duration > 0 && breakPoint >= duration / 2;
  }
...
export function songMatchesEditPointFilters(song: EditableSongLike, selectedTypes: string[]) {
